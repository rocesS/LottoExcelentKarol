...
class ResultCheckerFacadeTest {
    ...
    static RetrieverWonNumbersClient retrieverWonNumbersClient;
    ...
    static void init() {
        ...
        retrieverWonNumbersClient = mock(NumbersGeneratorFacadeTestImpl.class);
        resultCheckerFacade = new ResultCheckerFacadeConfiguration().createFacadeForTest(numberReceiverFacade, retrieverWonNumbersClient);
    ...
    void should_return_correct_lottery_results_with_winning_message_when_user_won() {
        //given
        LotteryTicketDto lotteryTicketDto = new LotteryTicketDto(UUID.randomUUID().toString(), (List.of(1, 2, 3, 4, 5, 6)),
                randomDateString, "valid");
        WinningNumbersDto winningNumbersDto = new WinningNumbersDto(UUID.randomUUID().toString(), List.of(1, 2, 3, 4, 5, 6), randomDateString);

        given(numberReceiverFacade.retrieveUserNumbers(any(String.class))).willReturn(lotteryTicketDto);
        given(retrieverWonNumbersClient.retrieveWonNumbers(randomDateString)).willReturn(winningNumbersDto);

        //when
        LotteryResultDto result = resultCheckerFacade.checkWinner(UUID.randomUUID().toString());
    ...
    void should_return_correct_lottery_results_with_losing_message_when_user_lost() {
        //given
        LotteryTicketDto lotteryTicketDto = new LotteryTicketDto(UUID.randomUUID().toString(), (List.of(1, 2, 8, 9, 10, 11)),
                randomDateString, "valid");
        WinningNumbersDto winningNumbersDto = new WinningNumbersDto(UUID.randomUUID().toString(), List.of(1, 2, 3, 4, 5, 6), randomDateString);
    ...
    void should_return_result_with_empty_credentials_and_invalid_message_when_user_gave_invalid_id() {
        ...
        given(numberReceiverFacade.retrieveUserNumbers(any(String.class))).willReturn(null);
        given(retrieverWonNumbersClient.retrieveWonNumbers(randomDateString)).willReturn(winningNumbersDto);
